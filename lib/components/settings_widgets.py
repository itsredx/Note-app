...
class SettingsTile(StatelessWidget):
    ...
    def build(self):
        ...
        if self.icon:
            ...
            children_row.append(
                SizedBox(
                    key=Key(f"{self.title}_{self.key}_tile_icon_padding"), 
                    width=12
                )
            )
        ...
        if self.subtitle:
            text_column_children.append(
                SizedBox(
                    key=Key(f"{self.title}_{self.key}_tile_subtitle_padding"),
                    height=4
                )
            )
            text_column_children.append(
        ...
        main_row_children = [
            Row(
                key=Key(f"{self.title}_{self.key}_tile_row"), children=children_row
    ...
    def build(self):
        ...
        return Container(
            ...
            decoration=BoxDecoration(
                ...
                border=BorderSide(
                    width=1, color=Colors.adaptive(dark="#5a5a5a", light="#d3d3d3")
                ),
            ),
            child=Column(
        ...
        return Container(
            ...
            child=Column(
                ...
                children=[
                    ...
                    SettingsCard(
                        key=Key(f"{self.key}_section_card"),
                        children=self.children_widgets,
                        last_item_margin=24 if self.last_item else 0,
                    ),
                ],
            ),
        )
